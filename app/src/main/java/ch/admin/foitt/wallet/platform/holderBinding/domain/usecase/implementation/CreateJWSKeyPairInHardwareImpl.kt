...
import ch.admin.foitt.wallet.platform.holderBinding.domain.model.CreateKeyGenSpecError
import ch.admin.foitt.wallet.platform.holderBinding.domain.model.KeyPairError
import ch.admin.foitt.wallet.platform.holderBinding.domain.model.toCreateJWSKeyPairError
...
internal class CreateJWSKeyPairInHardwareImpl @Inject constructor(
    ...
    private fun useStrongBox(keyStorageSecurityLevels: List<KeyStorageSecurityLevel>?): Result<Boolean, CreateJWSKeyPairError> = binding {
        ...
        if (keyStorageFiltered.isEmpty()) {
            return@binding Err(KeyPairError.UnsupportedKeyStorageSecurityLevel).bind<Boolean>()
        } else {
            ...
            return@binding Err(KeyPairError.IncompatibleDeviceKeyStorage).bind<Boolean>()
        }
    }
