...
class GenerateOcaClaimDisplaysImplTest {
    ...
    fun `Generating Oca claim displays with labels returns claim with displays`() = runTest {
        ...
        val result = useCase(CLAIM_KEY, CLAIM_VALUE, ocaClaimData)
    ...
    fun `Generating Oca claim displays with entries returns claim with displays`() = runTest {
        ...
        val result = useCase(CLAIM_KEY, ENTRY_CODE, ocaClaimData)
    ...
    fun `Generating Oca claim displays for data URL returns ImageType claim`(mimeType: String) = runTest {
        val ocaClaimData = createOcaClaimData(AttributeType.Text, standard = Standard.DataUrl)

        val result = useCase(CLAIM_KEY, "data:$mimeType;base64,$CLAIM_VALUE", ocaClaimData)

        assertClaim(result.first, value = CLAIM_VALUE, valueType = mimeType)
    }
    ...
    fun `Generating Oca claim displays for null data URL returns string claim`() = runTest {
        ...
        val result = useCase(CLAIM_KEY, null, ocaClaimData)
    ...
    @ParameterizedTest
    @ValueSource(strings = ["image/jpeg", "image/png"])
    fun `Generating Oca claim displays for binary Image returns ImageType claim`(mimeType: String) = runTest {
        val ocaClaimData =
    ...
    @ParameterizedTest
    @ValueSource(strings = ["image/jpeg", "image/png"])
    fun `Generating Oca claim displays for binary null value returns ImageType claim`(mimeType: String) = runTest {
        val ocaClaimData =
            createOcaClaimData(AttributeType.Binary, characterEncoding = CharacterEncoding.Base64, format = mimeType)

        val result = useCase(CLAIM_KEY, null, ocaClaimData)

        assertClaim(result.first, value = null, valueType = mimeType)
    }
    ...
    fun `Generating Oca claim displays for unknown data URL format returns string claim`() = runTest {
        ...
        val result = useCase(CLAIM_KEY, url, ocaClaimData)
    ...
    @ParameterizedTest
    @ValueSource(strings = ["image/jpeg", "image/png"])
    fun `Generating Oca claim displays for binary image without encoding returns string claim`(mimeType: String) = runTest {
        val ocaClaimData = createOcaClaimData(AttributeType.Binary, characterEncoding = null, format = mimeType)
    ...
    fun `Generating Oca claim displays for unix time returns date time claim`(
        ...
        val result = useCase(CLAIM_KEY, dateTime, ocaClaimData)
    ...
    fun `Generating Oca claim displays for unix time without standard returns unparsed date time claim`() = runTest {
        ...
        val result = useCase(CLAIM_KEY, "1749167999", ocaClaimData)
    ...
    fun `Generating Oca claim displays for ISO time without standard returns date time claim`() = runTest {
        ...
        val result = useCase(CLAIM_KEY, "2025-06-05T23:59:59+00:00", ocaClaimData)
    ...
    private fun assertClaim(
        ...
        assertEquals(CLAIM_KEY, claim.key)
