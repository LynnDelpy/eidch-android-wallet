...
class GenerateKeyPairImplTest {
    ...
    fun setUp() {
        ...
        val generateKeyPair = GenerateKeyPairImpl(
        ...
        every { spyGenerateKeyPair["getPreferredSigningAlgorithms"]() } returns listOf(
    ...
    fun `valid supported credential with software binding returns key pair with software type`() = runTest {
        val result = spyGenerateKeyPair(proofTypeConfigSoftwareBinding).assertOk()
    ...
    fun `valid supported credential with hardware binding returns key pair`() = runTest {
        val result = spyGenerateKeyPair(proofTypeConfigHardwareBinding).assertOk()
    ...
    fun `Supported proof type with multiple algorithms returns a key pair using the first matching algorithm of the preference list`(
        ...
        val result = spyGenerateKeyPair(ProofTypeConfig(issuerAlgorithms)).assertOk()
    ...
    fun `Supported proof type with supported algorithm that is not in the preference list returns an unsupported cryptographic suite error`() =
        runTest {
            ...
            spyGenerateKeyPair(proofTypeConfigSoftwareBinding).assertErrorType(KeyPairError.UnsupportedCryptographicSuite::class)
    ...
    fun `Generate key pair maps errors from createJWSKeyPair`() = runTest {
        ...
        spyGenerateKeyPair(proofTypeConfigHardwareBinding).assertErrorType(KeyPairError.Unexpected::class)
    }
    ...
    fun `Generate key pair maps errors from createJWSKeyPairInSoftware`() = runTest {
        ...
        spyGenerateKeyPair(proofTypeConfigSoftwareBinding).assertErrorType(KeyPairError.Unexpected::class)
    }
